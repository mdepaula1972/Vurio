...
export const AICopilotBar: React.FC<AICopilotBarProps> = ({ process, onAdvanceBlockedStep, onUpdateProcess }) => {
    ...
    <>
        ...
        <div className="rounded-2xl bg-slate-900/95 border border-indigo-500/40 p-4 sm:p-5 shadow-2xl backdrop-blur-xl flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 ring-1 ring-indigo-500/20">
          ...
          <div className="flex items-center space-x-2 w-full sm:w-auto justify-end relative z-10">
            <button
              ...
              className="px-3.5 py-2 rounded-xl bg-slate-800 hover:bg-slate-700 text-indigo-300 font-bold text-xs border border-slate-700 flex items-center space-x-1.5 transition-all"
            >
      ...
      <VoiceConsultantModal
        ...
        onConfirmConsultantFlow={({ title, description, creatorRole, hasFullAutonomy, prescribedSteps }) => {
          if (!onUpdateProcess) return;
          const generatedSteps = prescribedSteps.map((pStep, index) => ({
            id: `copilot-consult-${Date.now()}-${index}`,
